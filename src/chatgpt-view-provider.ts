		...
		webviewView.webview.onDidReceiveMessage(async data => {
			switch (data.type) {
				...
				case 'login':
					...
					break;
				case 'openSettings':
					vscode.commands.executeCommand('workbench.action.openSettings', "@ext:TaiyoFujii.vscode-chatgpt chatgpt");

					this.logEvent("settings-opened");
					break;
				case 'openSettingsPrompt':
					vscode.commands.executeCommand('workbench.action.openSettings', "@ext:TaiyoFujii.vscode-chatgpt promptPrefix");
						...
						<div class="flex items-start text-center features-block my-5">
							<div class="flex flex-col gap-3.5 flex-1">
								...
								<ul class="flex flex-col gap-3.5 text-xs">
									<li class="features-li w-full border border-zinc-700 p-3 rounded-md">コードの最適化、テストの追加、バグの捜索</li>
									<li class="features-li w-full border border-zinc-700 p-3 rounded-md">作成したコードの挿入、ファイル作成</li>
									<li class="features-li w-full border border-zinc-700 p-3 rounded-md">コードのハイライト</li>
								</ul>
							</div>
						</div>
						<div class="flex flex-col gap-4 h-full items-center justify-end text-center">
							<button id="login-button" class="mb-4 btn btn-primary flex gap-2 justify-center p-3 rounded-md">ログイン</button>
							<p class="max-w-sm text-center text-xs text-slate-500">
								<a title="" id="settings-button" href="#">プラグイン設定</a>&nbsp; | &nbsp;<a title="" id="settings-prompt-button" href="#">プロンプト設定</a>
							</p>
						</div>
					</div>
